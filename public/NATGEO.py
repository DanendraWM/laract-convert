...
for x in range(3, count_rows):
    ...
    program=str(sh.cell_value(rowx=x, colx=6)).capitalize()
    translation = translator.translate(program, dest="id")
    program_desc.append(translation.text)
    schedule_date=datetime.strptime(sh.cell_value(rowx=3, colx=0),'%d/%m/%Y').strftime('%Y.%m.%d')
    xl_time=datetime.strptime(sh.cell_value(rowx=3, colx=1),'%H:%M:%S')
    xl_end=datetime.strptime(sh.cell_value(rowx=3, colx=2),'%H:%M')
    end_time=(xl_time - time_zero + xl_end).time()
    date_start_time.append(f'{schedule_date} {sh.cell_value(rowx=x, colx=1)}')
    date_end_time.append(f'{schedule_date} {end_time}')
data={
