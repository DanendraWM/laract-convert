...
for x in range(1, count_rows):
    ...
    schedule_date=datetime.strptime(sh.cell_value(rowx=x, colx=0),'%Y-%m-%d').strftime('%Y.%m.%d')
    xl_time=datetime.strptime(sh.cell_value(rowx=x, colx=1),'%H:%M')
    xl_end=datetime.strptime(sh.cell_value(rowx=x, colx=2),'%H:%M')
